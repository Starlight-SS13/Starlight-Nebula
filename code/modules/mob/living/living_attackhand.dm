...
/mob/living/proc/try_extinguish(mob/living/user)
	...
	fire_intensity -= 0.5
	if (prob(10) && (user.fire_intensity <= 0))
		user.fire_intensity += 1
	user.ignite_fire()
	if (user.is_on_fire())
		user.visible_message(
	...
	fire_intensity -= 0.5 //Less effective than stop, drop, and roll - also accounting for the fact that it takes half as long.
	if (fire_intensity <= 0)
		user.visible_message(
